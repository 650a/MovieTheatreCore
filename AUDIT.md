...
**Location:** `com._650a.movietheatrecore.playback.PlaybackSession#stop`, `#tick`, `#renderFrame`, and the restart path in `PlaybackManager#start` (previously invoked `stop` with thumbnail refresh).
...
**Location:** `com._650a.movietheatrecore.render.ScalingMath#computeTransform` and `FrameScaler#scale` (git blame shows the regression introduced with the recent refactor adding `ScalingMath`).
...
Configuration is generated in `com._650a.movietheatrecore.configuration.Configuration#setup` (not from a bundled `config.yml`). Defaults currently include:  
...
`sources.allowed-domains`=[], `sources.max-download-mb`=1024, `sources.download-timeout-seconds`=30, `sources.cache-max-gb`=5, `sources.youtube-resolver-path`="", `sources.youtube-cookies-path`="", `sources.youtube-extra-args`=[], `sources.ffprobe-path`="plugins/MovieTheatreCore/runtime/bin/ffprobe", `sources.ffmpeg-path`="plugins/MovieTheatreCore/runtime/bin/ffmpeg".  
`dependencies.install.directory`="runtime/bin", `dependencies.install.exec-directory`="/home/container/MovieTheatreCore/runtime/bin", `dependencies.install.min-free-mb`=512, `dependencies.install.auto-install`=true, `dependencies.install.auto-update`=true.  
...
**Locations:** `src/com/_650a/movietheatrecore/configuration/Configuration.java#setup`.

### Current resolver implementations + external process execution

* YouTube resolver: `com._650a.movietheatrecore.media.MediaManager#resolveUrl` runs a configured resolver binary path (`sources.youtube-resolver-path`) using `ProcessBuilder`, with optional `--cookies` and extra args. It captures stdout/stderr after `waitFor()` to obtain a direct URL.  
* ffprobe metadata: `com._650a.movietheatrecore.ffmpeg.FFprobeService#probe` builds a CLI call to `plugin.getFfprobe().getExecutablePath()` and executes it with `ProcessBuilder` to parse JSON metadata.  
* ffmpeg processing: `com._650a.movietheatrecore.tasks.TaskAsyncLoadVideo#run` and `com._650a.movietheatrecore.stream.Stream#start` build `ProcessBuilder` commands to the ffmpeg executable, and `com._650a.movietheatrecore.audio.AudioPackManager#buildAudioPack` uses `ProcessBuilder` for audio pack generation.  
* Permissions hack: `com._650a.movietheatrecore.video.Video#generateVideo` and `com._650a.movietheatrecore.tasks.TaskAsyncLoadVideo#run` call `Runtime.getRuntime().exec("chmod -R 777 ...")` on ffmpeg/ffprobe files before running them.

### Expected binaries + current expected paths

* ffmpeg: `com._650a.movietheatrecore.ffmpeg.Ffmpeg#getExecutablePath` checks `sources.ffmpeg-path` or defaults to `<pluginData>/runtime/bin/ffmpeg` and falls back to `"ffmpeg"` on PATH.  
* ffprobe: `com._650a.movietheatrecore.ffmpeg.Ffprobe#getExecutablePath` checks `sources.ffprobe-path` or defaults to `<pluginData>/runtime/bin/ffprobe` and falls back to `"ffprobe"` on PATH.  
...
**Locations:** `src/com/_650a/movietheatrecore/ffmpeg/Ffmpeg.java#getExecutablePath`, `src/com/_650a/movietheatrecore/ffmpeg/Ffprobe.java#getExecutablePath`, `src/com/_650a/movietheatrecore/media/MediaManager.java#resolveUrl`.

### Known failure vectors (observed in reports)

* **NoClassDefFoundError net.bramp.ffmpeg.FFprobe:** The current codebase no longer uses the bramp FFprobe Java wrapper (replaced by `FFprobeService` calling the CLI). If older jars still include references, the stack trace would surface on metadata probing. Relevant metadata probing happens in `com._650a.movietheatrecore.ffmpeg.FFprobeService#probe` and call sites in `com._650a.movietheatrecore.video.Video#generateVideo`.  
* **Permission denied (noexec plugin dir):** ffmpeg/ffprobe are downloaded to `<pluginData>/runtime/bin` by `DependencyManager` and executed from there; if the plugin dir is `noexec`, configure `dependencies.install.exec-directory` to an exec-capable path.  
* **Read-only rootfs:** runtime downloads to `<pluginData>/runtime/bin` will fail if the plugin data dir is read-only; use `dependencies.install.exec-directory` on a writable volume.  
...
**Locations:** `src/com/_650a/movietheatrecore/Main.java#onEnable`, `src/com/_650a/movietheatrecore/ffmpeg/Ffmpeg.java#download`, `src/com/_650a/movietheatrecore/ffmpeg/Ffprobe.java#download`, `src/com/_650a/movietheatrecore/configuration/Configuration.java#getConfigFile`, `src/com/_650a/movietheatrecore/media/MediaManager.java#isAllowedUrl`.
