...
public class Scheduler {
    ...
    public BukkitTask runAsync(Runnable runnable) {
        if (!Bukkit.isPrimaryThread()) {
            Bukkit.getLogger().warning("[MovieTheatreCore]: runAsync invoked from an async thread. Verify threading assumptions.");
        }
    ...
    public BukkitTask runSyncRepeating(Runnable runnable, long delayTicks, long periodTicks) {
        if (!Bukkit.isPrimaryThread()) {
            Bukkit.getLogger().warning("[MovieTheatreCore]: runSyncRepeating invoked off the main thread.");
        }
    ...
    public BukkitTask runSyncLater(Runnable runnable, long delayTicks) {
        if (!Bukkit.isPrimaryThread()) {
            Bukkit.getLogger().warning("[MovieTheatreCore]: runSyncLater invoked off the main thread.");
        }
    ...
    public void ensureMainThread(String context) {
        if (!Bukkit.isPrimaryThread()) {
            Bukkit.getLogger().warning("[MovieTheatreCore]: Bukkit API call off the main thread (" + context + ").");
        }
    }
}
