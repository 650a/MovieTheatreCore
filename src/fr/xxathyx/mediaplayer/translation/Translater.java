...
public class Translater {
    ...
    public String ensureTranslationExported(String language) {
        ...
        if(!resourceExists(resourcePath)) {
            Bukkit.getLogger().warning("[MediaPlayer]: Missing bundled translation " + resolvedLanguage + ".yml, falling back to EN.");
            resolvedLanguage = "EN";
            resourcePath = resourcePathFor(resolvedLanguage);
        }

        if(!resourceExists(resourcePath)) {
            Bukkit.getLogger().warning("[MediaPlayer]: Missing bundled translation EN.yml; translations will not be exported.");
