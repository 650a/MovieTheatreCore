...
public class EmbeddedPackServer {
    ...
    public synchronized boolean start() {
        ...
        try {
            ...
            lastError = null;
            plugin.getLogger().info("[MediaPlayer]: Pack server started on " + boundHost + ":" + boundPort + ".");
            return true;
        } catch (BindException e) {
            lastError = "Port already in use (" + boundPort + ").";
            plugin.getLogger().warning("[MediaPlayer]: Failed to start pack server: " + lastError);
        } catch (IOException e) {
            lastError = e.getMessage();
            plugin.getLogger().warning("[MediaPlayer]: Failed to start pack server: " + e.getMessage());
        }
        server = null;
        return false;
    }
    ...
    public synchronized void stop() {
        if (server != null) {
            server.stop(0);
            server = null;
            plugin.getLogger().info("[MediaPlayer]: Pack server stopped.");
        }
    }
    ...
    private class FileHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            if (path == null || path.isBlank() || "/".equals(path)) {
                byte[] response = "MediaPlayer pack server".getBytes(java.nio.charset.StandardCharsets.UTF_8);
